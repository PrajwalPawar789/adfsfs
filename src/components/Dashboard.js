  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setUserData(response.data.userData.reverse()); // Reverse the userData array
        ...
        <div className="flex justify-center mt-4">
          <nav className="bg-gray-800 rounded-md">
            <ul className="flex p-3">
              <li>
                <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1} className="text-white hover:text-gray-300 font-semibold px-4 py-2 border-r border-gray-600 focus:outline-none">
                  Previous
                </button>
              </li>
              <li>
                <button onClick={() => paginate(currentPage + 1)} disabled={indexOfLastItem >= userData.length} className="text-white hover:text-gray-300 font-semibold px-4 py-2 focus:outline-none">
                  Next
                </button>
              </li>
            </ul>
